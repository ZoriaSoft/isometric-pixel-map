extends Control
## Main editor shell: themed UI, tools, about, file dialogs, splash, toasts.
...
@onready var btn_pen: Button = $UI/TopBar/Margin/HBox/BtnPen
@onready var btn_erase: Button = $UI/TopBar/Margin/HBox/BtnErase
@onready var btn_fill: Button = $UI/TopBar/Margin/HBox/BtnFill
@onready var btn_ground: Button = $UI/TopBar/Margin/HBox/BtnGround
@onready var btn_props: Button = $UI/TopBar/Margin/HBox/BtnProps
...
func _ready() -> void:
	...
	Game.tool_changed.connect(_refresh_tool_styles)
...
func _refresh_tool_styles() -> void:
	if btn_pen == null:
		return
	_set_tool_btn(btn_pen, Game.current_tool == Game.Tool.PEN)
	_set_tool_btn(btn_erase, Game.current_tool == Game.Tool.ERASE)
	_set_tool_btn(btn_fill, Game.current_tool == Game.Tool.FILL)
	_set_tool_btn(btn_ground, Game.active_layer == Palette.LAYER_GROUND)
	_set_tool_btn(btn_props, Game.active_layer == Palette.LAYER_PROPS)


func _set_tool_btn(btn: Button, active: bool) -> void:
	if btn == null:
		return
	var s := _style_active if active else _style_normal
...
func _on_map_changed() -> void:
	...
	match Game.current_tool:
		Game.Tool.PEN:
			tool_name = L.t("tool_pen")
		Game.Tool.ERASE:
			tool_name = L.t("tool_erase")
		Game.Tool.FILL:
			tool_name = L.t("tool_fill")
...
func _on_hover(cell: Vector2i) -> void:
	if status_label == null:
		return
	if Game.map and Game.map.in_bounds(cell.x, cell.y):
		...
		status_label.text = "%s · %d,%d · %s / %s" % [Game.map.title, cell.x, cell.y, g, prop_s]
